function pest(dataObject){
	...
	/* DELETE THIS CHUNK AFTER CONFIRMING THAT IT WORKS
	
	//Starting variables
	var currentIntensity = 0.7; //Starting intensity

	//Variables for Down/Up
	var downThreshold = 3; // 3 Down
	var upThreshold = 1; // 1 Up
	var upperIntensityLimit = 1.0;
	var lowerIntensityLimit = 0.0000000001; //Practically 0, but cannot set to 0 because JavaScript evaluates it as false in the RDK plugin
	...
	//Variables for PEST
	var currentStepSize = 0.32; //Starting step size
	...
	//Function to intialize the PEST variables
	function initializePest(dataObject){
		currentIntensity = dataObject.starting_intensity || 0.7;
		downThreshold = dataObject.down_threshold || 3;
		upThreshold = dataObject.up_threshold || 1;
		upperIntensityLimit = dataObject.upper_intensity_limit || 1;
		lowerIntensityLimit = dataObject.lower_intensity_limit || 0;
		currentStepSize = dataObject.starting_step_size || 0.32;
		minimumStepSize = dataObject.min_step_size || 0.01;
		maximumStepSize = dataObject.max_step_size || 0.32;
	}
